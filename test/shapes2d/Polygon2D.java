...
public class Polygon2D extends Shape2D {
     ...
     */
    public Polygon2D(float posX, float posY, float size, int color, Vec2df... points) {
        super(posX, posY, color);
        this.size = size;
        pShape = new ArrayList<>();
        pFinal = new ArrayList<>();
        for ( Vec2df point : points ) {
            pShape.add(point);
            pFinal.add(point);
        }
     ...
     * @param color El color del polígno.
     */
    public Polygon2D(float posX, float posY, ArrayList<Vec2df> p, float size, int color) {
        super(posX, posY, color);
        this.size = size;
        pShape = p;
        pFinal = p;
    ...
    public Polygon2D(float posX, float posY, float size, int numVertex, int color) {
        ...
        for (int i = 0; i < this.numVertex; i++) {
            pShape.add(new Vec2df((float)(this.size * Math.cos(theta * i)), (float)(this.size * Math.sin(theta * i))));
            pFinal.add(new Vec2df((float)(this.size * Math.cos(theta * i)), (float)(this.size * Math.sin(theta * i))));
        }
        normalizePShape();
        calculateTheoreticalCenter();
        rotateScaleOffsetPoints();
    ...
    protected void calculateTheoreticalCenter(){
        ...
        sumX /= numVertex;
        sumY /= numVertex;
        theoreticalCenter = new Vec2df(sumX, sumY);
    }
    ...
    protected void rotateScaleOffsetPoints() {
        for (int i = 0; i < numVertex; i++ ) {
            Vec2df point = new Vec2df(pShape.get(i).getX(), pShape.get(i).getY());
        ...
        if ( pFinal.size() > 4 ) {
            for ( int i = 2; i <= pFinal.size() -2; i++ ) {
                r.drawLine(
                        ...
                        (int)pFinal.get(i).getX(), (int)pFinal.get(i).getY(),
                        color);
            }
        }
    }
    ...
    public void drawYourSelf(Renderer r) {
        switch ( wayToRender) {
            ...
            case BLUEPRINT:
                ...
                r.drawLine(
                        (int)pFinal.get(0).getX(), (int)pFinal.get(0).getY(),
                        (int)(theoreticalCenter.getX() + posX), (int)(theoreticalCenter.getY() + posY),
                        0xffffffff);
                r.drawCircle((int)(posX), (int)(posY), 2, 0xffffffff);
                r.drawFillCircle((int)(theoreticalCenter.getX() + posX), (int)(theoreticalCenter.getY() + posY),
                        2, 0xffffffff);
                break;
            case BLACKBOARD:
    ...
    public ArrayList<Vec2df> getP() {
        return pFinal;
    }

    public void addP(Vec2df p) {
        pShape.add(p);
        pFinal.add(p);
        normalizePShape();
        calculateTheoreticalCenter();
    ...
    public void setP(ArrayList<Vec2df> p) {
        this.pShape = p;
        this.pFinal = p;
        normalizePShape();
        calculateTheoreticalCenter();
